...
#
# Uploaded CSVs MUST match this schema — same column names, same categorical
# values. This keeps preprocessing predictable and the model reliable.
# ─────────────────────────────────────────────────────────────────────────────
ID_COL      = 'patient_id'
...
NUMERIC_COLS = [
    ...
    'num_previous_admissions', 'medications_count',
    'last_hemoglobin', 'last_glucose', 'last_creatinine',
    'length_of_stay', 'procedures_count',
]

CATEGORICAL_COLS = [
    'gender', 'chronic_conditions', 'admission_type',
    'smoking_status', 'alcohol_use', 'physical_activity',
    'insurance_type', 'followup_compliance',
    'social_support', 'mental_health_issue',
]

# Allowed values per categorical column (case-sensitive).
# NaN is allowed for columns that legitimately have missing values.
CATEGORY_VALUES = {
    'gender':              ['Female', 'Male'],
...
NUMERIC_PLAUSIBLE_RANGE = {
    'age':                      (0,   120),
    'weight_kg':                (20,  300),
    'height_cm':                (100, 230),
    'bmi':                      (10,  70),
    'num_previous_admissions':  (0,   30),
    'medications_count':        (0,   40),
    'last_hemoglobin':          (4,   22),
    'last_glucose':             (40,  600),
    'last_creatinine':          (0.2, 15),
    'length_of_stay':           (0,   120),
    'procedures_count':         (0,   20),
}

EXPECTED_COLUMNS = [ID_COL] + NUMERIC_COLS + CATEGORICAL_COLS + [TARGET_COL]

# Prefixes generated by one-hot encoding — used to pretty-print feature names.
ONEHOT_PREFIX_LABEL = {
    'gender_':              'Gender',
    'chronic_conditions_':  'Chronic condition',
    'admission_type_':      'Admission type',
    'smoking_status_':      'Smoking status',
    'alcohol_use_':         'Alcohol use',
    'physical_activity_':   'Physical activity',
    'insurance_type_':      'Insurance',
    'followup_compliance_': 'Follow-up compliance',
    'social_support_':      'Social support',
    'mental_health_issue_': 'Mental health issue',
}
...
_defaults = dict(
    ...
    # ─── New-patient (ad-hoc scoring) state ───
    new_patient_result=None,            # dict with prob, tier, shap, inputs for the last scored new patient
    new_patient_extras=[],              # list of dicts for new patients added to the roster this session
...
def preprocess(df, target_col):
    """Schema-aware preprocessing for the clinical readmission dataset.

    This preprocessor trusts the schema contract declared at the top of the
    file. It performs:

      • Drop patient_id if present (identifier, never a feature)
      • Coerce NUMERIC_COLS to numeric, clip to NUMERIC_PLAUSIBLE_RANGE to
        remove nonsense outliers, then median-impute remaining NaNs
      • Fill categorical NaNs with 'Unknown', then one-hot encode (drop_first
        off — we want an explicit dummy per category for SHAP interpretability)
      • Build a binary __target__: 1 if target_col == POS_CLASS ('High'), else 0

    Returns (df_processed, feature_names, cat_cols, num_cols, le_dict, pos_label).
    `le_dict` is kept for API compatibility (empty dict now — replaced by
    one-hot encoding).

    If the dataframe does not match the expected schema, this function falls
    back to a generic heuristic path so the technical view still works on
    arbitrary CSVs. Strict enforcement lives in `validate_schema`.
    """
    df = df.copy()

    # ── Strict path: dataframe matches the clinical schema ───────────────
    schema = validate_schema(df) if target_col == TARGET_COL else {'ok': False}
    if schema['ok'] and target_col == TARGET_COL:
        ...
        pos_label = POS_CLASS
        df['__target__'] = (target_series == POS_CLASS).astype(int)

        # Numerics: coerce → clip to plausible range → median impute
        ...
        # Categoricals: fill NaN with 'Unknown' → one-hot
        cat_cols = [c for c in CATEGORICAL_COLS if c in df.columns]
        for c in cat_cols:
            df[c] = df[c].astype(str).replace({'nan': 'Unknown', 'NaN': 'Unknown', 'None': 'Unknown'})
            df[c] = df[c].fillna('Unknown')
        if cat_cols:
            df = pd.get_dummies(df, columns=cat_cols, drop_first=False, dtype=float)

        # Feature names = all columns except target & bookkeeping
        feats = [c for c in df.columns if c not in (target_col, '__target__')]

        # Ensure all feature columns are numeric (get_dummies gives floats)
        for c in feats:
            if not pd.api.types.is_numeric_dtype(df[c]):
                df[c] = pd.to_numeric(df[c], errors='coerce').fillna(0.0)

        return df, feats, cat_cols, num_cols, {}, pos_label

    # ── Fallback (generic CSVs from the technical view) ──────────────────
...
def train_models(X_tr, y_tr, X_te, y_te):
    """Train three classifiers with stratified 5-fold CV for reliable model
    selection, then calibrate the best model with isotonic regression.

    For each base model:
      • Run 5-fold StratifiedKFold cross-validation on the training set to get
        an unbiased AUC estimate (logged as cv_auc)
      • Fit on the full training set
      • Report held-out test metrics (auc, ap, f1, acc, brier, bal)

    For the best model by cv_auc, we additionally fit a CalibratedClassifierCV
    wrapper (isotonic, 5-fold) so that downstream probability scores are well
    calibrated — critical for clinical risk thresholds. The calibrated model
    replaces the raw model in the returned dict, but the raw model is kept
    under `raw_model` so the technical view can still compare them.
    """
    ...
    models_cfg = {
        ...
        'XGBoost': xgb.XGBClassifier(
            ...
            subsample=0.85, colsample_bytree=0.85, scale_pos_weight=scale_pos,
    ...
    # Pass 1: fit each base model on the full training set + CV-score it.
    for name, model in models_cfg.items():
        try:
            cv_auc = cross_val_score(
                clone(model), X_tr, y_tr, cv=cv, scoring='roc_auc', n_jobs=-1,
            )
            model.fit(X_tr, y_tr)
    ...
    # Pass 2: calibrate the best-by-cv model (isotonic, internal 5-fold CV).
    best_name = max(results, key=lambda k: results[k]['cv_auc'])
    try:
        best_base = clone(models_cfg[best_name])
        calibrated = CalibratedClassifierCV(best_base, method='isotonic', cv=5)
        calibrated.fit(X_tr, y_tr)
        cal_probs = calibrated.predict_proba(X_te)[:, 1]
        cal_preds = (cal_probs >= 0.5).astype(int)
        # Replace the active model with the calibrated one so downstream
        # scoring uses well-calibrated probabilities.
        results[best_name]['model'] = calibrated
        results[best_name]['probs'] = cal_probs
        results[best_name]['preds'] = cal_preds
        results[best_name]['auc']   = roc_auc_score(y_te, cal_probs)
        results[best_name]['ap']    = average_precision_score(y_te, cal_probs)
        results[best_name]['f1']    = f1_score(y_te, cal_preds, zero_division=0)
        results[best_name]['acc']   = accuracy_score(y_te, cal_preds)
        results[best_name]['brier'] = brier_score_loss(y_te, cal_probs)
        results[best_name]['bal']   = balanced_accuracy_score(y_te, cal_preds)
        results[best_name]['calibrated'] = True
    except Exception as e:
        st.warning(f'Calibration for {best_name} skipped – {e}')
        results[best_name]['calibrated'] = False

    return results

def compute_shap(model_name, model, X_te, feature_names):
    """Compute SHAP values for the chosen model, safely unwrapping
    CalibratedClassifierCV and Pipeline layers to reach the base estimator.
    """
    try:
        clf = model
        X_shap = X_te

        # Unwrap CalibratedClassifierCV first (it averages across folds — use fold 0).
        if hasattr(clf, 'calibrated_classifiers_'):
            try:
                clf = clf.calibrated_classifiers_[0].estimator
            except AttributeError:
                # Older sklearn attribute name
                clf = clf.calibrated_classifiers_[0].base_estimator

        # Then unwrap any Pipeline by applying pre-processing steps to X.
...
def humanize_feature(feat_name: str) -> str:
    ...
    Handles one-hot expanded names (e.g. 'admission_type_Emergency' →
    'Admission type: Emergency') and the generic FEATURE_PLAIN_LANGUAGE map.
    """
    if feat_name is None:
        return 'Clinical factor'
    name = str(feat_name)
    # One-hot encoded name like 'chronic_conditions_Diabetes'
    for prefix, label in ONEHOT_PREFIX_LABEL.items():
...
def validate_schema(df: pd.DataFrame) -> dict:
    """Validate an uploaded dataframe against the reference clinical schema.

    Returns a dict with keys:
      ok            — True if the dataframe is safe to train on
      missing       — columns required but absent
      extra         — columns present that aren't expected
      bad_values    — [(column, [unexpected_values...]), ...] for categoricals
      out_of_range  — [(column, n_rows_outside_plausible_range), ...]
      messages      — user-facing bullet lines describing every problem
    """
    ...
    bad_values = []
    for c, allowed in CATEGORY_VALUES.items():
        if c not in df.columns:
            continue
        present = set(df[c].dropna().astype(str).unique())
        unexpected = sorted(present - set(allowed))
        if unexpected:
            bad_values.append((c, unexpected))

    # Target must be binary with known labels
    target_bad = False
    if TARGET_COL in df.columns:
        vals = set(df[TARGET_COL].dropna().astype(str).unique())
        target_bad = not vals.issubset({POS_CLASS, NEG_CLASS})

    out_of_range = []
    for c, (lo, hi) in NUMERIC_PLAUSIBLE_RANGE.items():
        if c not in df.columns:
            continue
        col = pd.to_numeric(df[c], errors='coerce')
        n_bad = int(((col < lo) | (col > hi)).sum())
        if n_bad:
            out_of_range.append((c, n_bad, lo, hi))

    messages = []
    if missing:
        messages.append(f"Missing required columns: **{', '.join(missing)}**")
    if extra:
        messages.append(f"Unexpected extra columns: **{', '.join(extra)}**")
    for col, vals in bad_values:
        allowed = CATEGORY_VALUES[col]
        messages.append(
            f"Column **{col}** contains unexpected values "
            f"({', '.join(vals[:5])}{'…' if len(vals) > 5 else ''}) — "
            f"allowed: {', '.join(allowed)}."
        )
    if target_bad:
        messages.append(
            f"Column **{TARGET_COL}** must contain only "
            f"`{POS_CLASS}` or `{NEG_CLASS}`."
        )
    for col, n_bad, lo, hi in out_of_range:
        messages.append(
            f"Column **{col}** has {n_bad} values outside the plausible clinical "
            f"range [{lo}, {hi}] — these will be clipped during preprocessing."
        )

    ok = not (missing or extra or bad_values or target_bad)
    return {
        'ok': ok,
        'missing': missing,
        'extra': extra,
...
def score_new_patient(patient_dict, feats, model, explainer=None, model_name=None):
    """Score a single ad-hoc patient entered through the manual form.

    Builds a 1-row DataFrame from the form inputs, runs it through the same
    preprocess() pipeline used for the uploaded dataset, aligns the resulting
    columns to the trained feature space (filling any missing one-hot columns
    with 0), and returns the probability plus SHAP values for that single row.

    Returns (prob, shap_values_1d, processed_row_df). shap_values_1d may be
    None if the explainer is not available or fails on this single row.
    """
    # Build a 1-row dataframe with the schema columns. We add a dummy target
    # so preprocess() takes the strict schema-aware path.
    row = {ID_COL: 'NEW_PATIENT'}
    for c in NUMERIC_COLS:
        row[c] = patient_dict.get(c, np.nan)
    for c in CATEGORICAL_COLS:
        row[c] = patient_dict.get(c, 'Unknown')
    row[TARGET_COL] = NEG_CLASS  # placeholder — discarded after preprocess
    df_single = pd.DataFrame([row])

    # Run through the same preprocessing pipeline used for the uploaded data.
    df_p, _, _, _, _, _ = preprocess(df_single, TARGET_COL)

    # Align columns to the trained feature space. One-hot encoding on a
    # single row only produces dummies for the categories actually present,
    # so any column the model was trained on must be added with value 0.
    X_aligned = pd.DataFrame(0.0, index=df_p.index, columns=feats)
    for c in feats:
        if c in df_p.columns:
            X_aligned[c] = pd.to_numeric(df_p[c], errors='coerce').fillna(0.0)

    X_arr = X_aligned.values
    prob = float(model.predict_proba(X_arr)[0, 1])

    # SHAP for the new patient. Reuse the existing explainer when possible.
    sv_single = None
    if explainer is not None:
        try:
            # Unwrap calibration / pipeline the same way compute_shap does,
            # so we transform X consistently with how the explainer was built.
            clf = model
            X_for_shap = X_arr
            if hasattr(clf, 'calibrated_classifiers_'):
                try:
            ...
            if isinstance(clf, Pipeline):
                pre = Pipeline(clf.steps[:-1])
                X_for_shap = pre.transform(X_arr)
            ...
            if sv.ndim == 3:
                sv = sv[:, :, 1]
            sv_single = sv[0] if sv.ndim == 2 else sv
        except Exception:
            sv_single = None

    return prob, sv_single, X_aligned
    ...
    if S.view_mode == 'technical':
        ...
        if uploaded_file is not None:
            try:
                ...
                if not schema['ok']:
                    st.warning(
                        f"⚠️ **{uploaded_file.name}** does not match the expected "
                        f"clinical schema. The generic preprocessor will be used instead "
                        f"— results may be less reliable. Issues:"
                    )
                    for msg in schema['messages']:
                        if 'outside the plausible clinical' not in msg:
                            st.markdown(f"- {msg}")
                elif schema['out_of_range']:
                    st.info(
                        "Some values are outside plausible clinical ranges and will be "
                        "clipped during preprocessing."
                    )
                S.df = df_raw
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if S.df is None:
        ...
        with st.expander("📋  What does my file need to look like?", expanded=False):
            st.markdown(
                "Your CSV must contain the following columns, using these exact names "
                "(case-sensitive):"
            )
            schema_rows = []
            schema_rows.append(('patient_id', 'Identifier (any string or number)'))
            for c in NUMERIC_COLS:
                lo, hi = NUMERIC_PLAUSIBLE_RANGE[c]
                schema_rows.append((c, f'Number between {lo} and {hi}'))
            for c in CATEGORICAL_COLS:
                schema_rows.append((c, 'One of: ' + ', '.join(CATEGORY_VALUES[c])))
            schema_rows.append((TARGET_COL, f'"{POS_CLASS}" or "{NEG_CLASS}"'))
            st.dataframe(
                pd.DataFrame(schema_rows, columns=['Column', 'Expected values']),
        ...
        if up is not None:
            try:
                ...
                schema = validate_schema(df_uploaded)
                if not schema['ok']:
                    st.markdown(
                        "<div class='warn-box' style='padding:14px 18px;margin-top:12px;'>"
                        "<strong>We couldn't accept this file.</strong> The columns or values "
                        "don't match the expected clinical dataset format. Please fix the issues "
                        "below and try again."
                        "</div>",
                        unsafe_allow_html=True,
                    )
                    for msg in schema['messages']:
                        st.markdown(f"- {msg}")
                    st.caption(
                        "Expand the section above to see the exact column names and allowed values."
                    )
                else:
                    S.df = df_uploaded
                    S.analysis_done = False
                    ...
                    S.all_shap = None
                    S.clinical_selected_idx = None
                    if schema['out_of_range']:
                        st.info(
                            "Note: some values were outside plausible clinical ranges and "
                            "will be gently clipped during analysis."
                        )
                    st.rerun()
            except Exception as e:
    ...
    # ─────────── STATE 3: Analysis complete — roster / drill-in ───────────────
        ...
        if S.clinical_selected_idx is not None and 0 <= S.clinical_selected_idx < n_total:
            ...
            st.markdown("<div class='panel'>", unsafe_allow_html=True)
            st.markdown("<div class='panel-title'>Where this patient sits in the cohort</div>",
                        unsafe_allow_html=True)
            st.markdown(
                "<div class='panel-sub'>This patient's risk score compared to every other "
                "patient in the uploaded dataset.</div>",
                unsafe_allow_html=True,
            )
            cohort_pct = np.asarray(S.all_probs) * 100
            ...
            fig_pos.add_vline(
                x=this_pct, line_color=color, line_width=3,
                annotation_text=f'This patient ({this_pct:.1f}%)',
                annotation_position='top',
                annotation_font_color=color,
                annotation_font_size=12,
            )
            fig_pos.add_vline(
                x=thr * 100, line_dash='dash', line_color='#475569', line_width=1.5,
                annotation_text=f'Threshold ({thr * 100:.0f}%)',
            ...
            fig_pos.update_layout(
                barmode='stack',
                height=300,
                xaxis_title='30-day readmission risk (%)',
            ...
            fig_pos.update_xaxes(gridcolor='#e2e8f0', zeroline=False)
            fig_pos.update_yaxes(gridcolor='#e2e8f0', zeroline=False)
            st.plotly_chart(fig_pos, use_container_width=True)
            st.caption(
                f"This patient scores higher than approximately **{percentile:.0f}%** "
                f"of the cohort."
            )
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        # ── Otherwise, render the roster dashboard ──────────────────────────
            ...
            # ═════════════════ COHORT INSIGHTS ═════════════════════════════
            st.markdown("<div class='panel'>", unsafe_allow_html=True)
            st.markdown("<div class='panel-title'>Cohort insights</div>", unsafe_allow_html=True)
            st.markdown(
                "<div class='panel-sub'>Visual overview of readmission risk "
                "across the entire uploaded patient cohort.</div>",
                unsafe_allow_html=True,
            )
            ...
            # ── Viz 1: Cohort risk distribution ──────────────────────────────
            with viz_tab1:
                risk_pct = probs * 100
                fig_dist = go.Figure()
                for tier_name, tcolor in [('Low', LOW_COLOR), ('Medium', MED_COLOR), ('High', HIGH_COLOR)]:
                ...
                fig_dist.add_vline(
                    x=thr * 100, line_dash='dash', line_color='#1e293b', line_width=2,
                    annotation_text=f'Threshold ({thr * 100:.0f}%)',
                    annotation_position='top',
                )
                ...
                fig_dist.update_xaxes(gridcolor='#e2e8f0', zeroline=False)
                fig_dist.update_yaxes(gridcolor='#e2e8f0', zeroline=False)
            ...
            with viz_tab2:
                if S.shap_vals is not None and S.feature_names:
                    ...
                    for feat, val in zip(S.feature_names, mean_abs):
                        base = feat
                        for prefix in ONEHOT_PREFIX_LABEL:
                    ...
                    fig_drv = go.Figure(go.Bar(
                        ...
                        marker=dict(
                            ...
                            line=dict(color='#0369a1', width=0.5),
                        ),
                        text=[f'{v:.3f}' for v in values],
                        textposition='outside',
                        hovertemplate='%{y}<br>Mean impact: %{x:.4f}<extra></extra>',
                    ))
                    ...
                    fig_drv.update_yaxes(zeroline=False)
                    st.plotly_chart(fig_drv, use_container_width=True)
                    st.caption(
                        "The clinical factors with the largest average influence on risk "
                        "predictions across this cohort."
                    )
                else:
                    st.info("Driver analysis unavailable for this cohort.")

            # ── Viz 3: Risk by subgroup ──────────────────────────────────────
            with viz_tab3:
                subgroup_choices = []
                age_col_name = next((c for c in df.columns if c.lower() == 'age'), None)
                if age_col_name is not None:
                    subgroup_choices.append('Age band')
                for c in ['admission_type', 'insurance_type', 'chronic_conditions',
                          'smoking_status', 'gender']:
                    if c in df.columns:
                        subgroup_choices.append(c)
                if not subgroup_choices:
                    ...
                    if sub.empty:
                        ...
                        sub_agg = sub.groupby('group', observed=True).agg(
                            mean_risk=('risk', 'mean'),
                            n=('risk', 'size'),
                        ...
                        fig_sub.add_trace(go.Bar(
                            x=sub_agg['pct_high'], y=sub_agg['group'].astype(str),
            ...
            # ═════════════════ SCORE A NEW PATIENT ══════════════════════════
            st.markdown("<div class='panel'>", unsafe_allow_html=True)
            st.markdown("<div class='panel-title'>Score a new patient</div>", unsafe_allow_html=True)
            st.markdown(
                "<div class='panel-sub'>Enter a patient's attributes to get an "
                "ad-hoc readmission risk score using the trained model. Useful "
                "for bedside decision support when the patient isn't in the uploaded file.</div>",
                unsafe_allow_html=True,
            )

            with st.expander("➕  Open new patient form", expanded=False):
                # Cohort medians/modes as smart defaults so the form starts in a
                # "typical patient" state and the clinician only changes what matters.
                num_defaults = {}
                for c in NUMERIC_COLS:
                    if c in df.columns:
                        ...
                        if pd.isna(med):
                            med = (lo + hi) / 2.0
                        num_defaults[c] = float(np.clip(med, lo, hi))
                cat_defaults = {}
                for c in CATEGORICAL_COLS:
                    if c in df.columns:
                        try:
                            cat_defaults[c] = str(df[c].mode().iloc[0])
                        except Exception:
                            cat_defaults[c] = CATEGORY_VALUES[c][0]
                    else:
                        cat_defaults[c] = CATEGORY_VALUES[c][0]

                with st.form("new_patient_form"):
                    st.markdown("**Patient identifier**")
                    new_pid = st.text_input(
                        "Patient ID (optional)",
                        value=f"NEW-{datetime.now().strftime('%H%M%S')}",
                        help="A label for this patient — does not affect the score.",
                    )

                    st.markdown("**Demographics & vitals**")
                    c1, c2, c3 = st.columns(3)
                    inputs = {}
                    inputs['age'] = c1.number_input(
                        "Age (years)",
                        min_value=float(NUMERIC_PLAUSIBLE_RANGE['age'][0]),
                        max_value=float(NUMERIC_PLAUSIBLE_RANGE['age'][1]),
                        value=num_defaults.get('age', 65.0), step=1.0,
                    )
                    inputs['gender'] = c2.selectbox(
                        "Gender", CATEGORY_VALUES['gender'],
                        index=CATEGORY_VALUES['gender'].index(cat_defaults['gender'])
                            if cat_defaults['gender'] in CATEGORY_VALUES['gender'] else 0,
                    )
                    inputs['weight_kg'] = c3.number_input(
                        "Weight (kg)",
                        min_value=float(NUMERIC_PLAUSIBLE_RANGE['weight_kg'][0]),
                        max_value=float(NUMERIC_PLAUSIBLE_RANGE['weight_kg'][1]),
                        value=num_defaults.get('weight_kg', 75.0), step=0.5,
                    )
                    c4, c5, c6 = st.columns(3)
                    inputs['height_cm'] = c4.number_input(
                        "Height (cm)",
                        min_value=float(NUMERIC_PLAUSIBLE_RANGE['height_cm'][0]),
                        max_value=float(NUMERIC_PLAUSIBLE_RANGE['height_cm'][1]),
                        value=num_defaults.get('height_cm', 170.0), step=0.5,
                    )
                    inputs['bmi'] = c5.number_input(
                        "BMI",
                        min_value=float(NUMERIC_PLAUSIBLE_RANGE['bmi'][0]),
                        max_value=float(NUMERIC_PLAUSIBLE_RANGE['bmi'][1]),
                        value=num_defaults.get('bmi', 26.0), step=0.1,
                    )
                    inputs['admission_type'] = c6.selectbox(
                        "Admission type", CATEGORY_VALUES['admission_type'],
                        index=CATEGORY_VALUES['admission_type'].index(cat_defaults['admission_type'])
                            if cat_defaults['admission_type'] in CATEGORY_VALUES['admission_type'] else 0,
                    )

                    st.markdown("**Hospitalisation & medication history**")
                    c7, c8, c9 = st.columns(3)
                    inputs['length_of_stay'] = c7.number_input(
                        "Length of stay (days)",
                        min_value=float(NUMERIC_PLAUSIBLE_RANGE['length_of_stay'][0]),
                        max_value=float(NUMERIC_PLAUSIBLE_RANGE['length_of_stay'][1]),
                        value=num_defaults.get('length_of_stay', 4.0), step=1.0,
                    )
                    inputs['num_previous_admissions'] = c8.number_input(
                        "Prior admissions",
                        min_value=int(NUMERIC_PLAUSIBLE_RANGE['num_previous_admissions'][0]),
                        max_value=int(NUMERIC_PLAUSIBLE_RANGE['num_previous_admissions'][1]),
                        value=int(num_defaults.get('num_previous_admissions', 1)), step=1,
                    )
                    inputs['medications_count'] = c9.number_input(
                        "Medications count",
                        min_value=int(NUMERIC_PLAUSIBLE_RANGE['medications_count'][0]),
                        max_value=int(NUMERIC_PLAUSIBLE_RANGE['medications_count'][1]),
                        value=int(num_defaults.get('medications_count', 5)), step=1,
                    )
                    c10, c11, c12 = st.columns(3)
                    inputs['procedures_count'] = c10.number_input(
                        "Procedures count",
                        min_value=int(NUMERIC_PLAUSIBLE_RANGE['procedures_count'][0]),
                        max_value=int(NUMERIC_PLAUSIBLE_RANGE['procedures_count'][1]),
                        value=int(num_defaults.get('procedures_count', 1)), step=1,
                    )
                    inputs['last_hemoglobin'] = c11.number_input(
                        "Last hemoglobin (g/dL)",
                        min_value=float(NUMERIC_PLAUSIBLE_RANGE['last_hemoglobin'][0]),
                        max_value=float(NUMERIC_PLAUSIBLE_RANGE['last_hemoglobin'][1]),
                        value=num_defaults.get('last_hemoglobin', 13.0), step=0.1,
                    )
                    inputs['last_glucose'] = c12.number_input(
                        "Last glucose (mg/dL)",
                        min_value=float(NUMERIC_PLAUSIBLE_RANGE['last_glucose'][0]),
                        max_value=float(NUMERIC_PLAUSIBLE_RANGE['last_glucose'][1]),
                        value=num_defaults.get('last_glucose', 110.0), step=1.0,
                    )
                    c13, c14, _ = st.columns(3)
                    inputs['last_creatinine'] = c13.number_input(
                        "Last creatinine (mg/dL)",
                        min_value=float(NUMERIC_PLAUSIBLE_RANGE['last_creatinine'][0]),
                        max_value=float(NUMERIC_PLAUSIBLE_RANGE['last_creatinine'][1]),
                        value=num_defaults.get('last_creatinine', 1.0), step=0.1,
                    )
                    inputs['chronic_conditions'] = c14.selectbox(
                        "Chronic condition", CATEGORY_VALUES['chronic_conditions'],
                        index=CATEGORY_VALUES['chronic_conditions'].index(cat_defaults['chronic_conditions'])
                            if cat_defaults['chronic_conditions'] in CATEGORY_VALUES['chronic_conditions'] else 0,
                    )

                    st.markdown("**Lifestyle & social factors**")
                    c15, c16, c17 = st.columns(3)
                    inputs['smoking_status'] = c15.selectbox(
                        "Smoking status", CATEGORY_VALUES['smoking_status'],
                        index=CATEGORY_VALUES['smoking_status'].index(cat_defaults['smoking_status'])
                            if cat_defaults['smoking_status'] in CATEGORY_VALUES['smoking_status'] else 0,
                    )
                    inputs['alcohol_use'] = c16.selectbox(
                        "Alcohol use", CATEGORY_VALUES['alcohol_use'],
                        index=CATEGORY_VALUES['alcohol_use'].index(cat_defaults['alcohol_use'])
                            if cat_defaults['alcohol_use'] in CATEGORY_VALUES['alcohol_use'] else 0,
                    )
                    inputs['physical_activity'] = c17.selectbox(
                        "Physical activity", CATEGORY_VALUES['physical_activity'],
                        index=CATEGORY_VALUES['physical_activity'].index(cat_defaults['physical_activity'])
                            if cat_defaults['physical_activity'] in CATEGORY_VALUES['physical_activity'] else 0,
                    )
                    c18, c19, c20 = st.columns(3)
                    inputs['insurance_type'] = c18.selectbox(
                        "Insurance type", CATEGORY_VALUES['insurance_type'],
                        index=CATEGORY_VALUES['insurance_type'].index(cat_defaults['insurance_type'])
                            if cat_defaults['insurance_type'] in CATEGORY_VALUES['insurance_type'] else 0,
                    )
                    inputs['followup_compliance'] = c19.selectbox(
                        "Follow-up compliance", CATEGORY_VALUES['followup_compliance'],
                        index=CATEGORY_VALUES['followup_compliance'].index(cat_defaults['followup_compliance'])
                            if cat_defaults['followup_compliance'] in CATEGORY_VALUES['followup_compliance'] else 0,
                    )
                    inputs['social_support'] = c20.selectbox(
                        "Social support", CATEGORY_VALUES['social_support'],
                        index=CATEGORY_VALUES['social_support'].index(cat_defaults['social_support'])
                            if cat_defaults['social_support'] in CATEGORY_VALUES['social_support'] else 0,
                    )
                    c21, c22, _ = st.columns(3)
                    inputs['mental_health_issue'] = c21.selectbox(
                        "Mental health issue", CATEGORY_VALUES['mental_health_issue'],
                        index=CATEGORY_VALUES['mental_health_issue'].index(cat_defaults['mental_health_issue'])
                            if cat_defaults['mental_health_issue'] in CATEGORY_VALUES['mental_health_issue'] else 0,
                    )
                    add_to_roster = c22.checkbox(
                        "Add this patient to the roster after scoring",
                        value=False,
                        help="If checked, this ad-hoc patient will appear in the roster table below.",
                    )

                    submitted = st.form_submit_button("🎯  Score this patient", use_container_width=True)

                if submitted:
                    # Pull the trained best model from the analysis bundle.
                    nm = S.best_name
                    model = S.models[nm]
                    feats = S.feature_names

                    try:
                        prob, sv_new, X_row = score_new_patient(
                            inputs, feats, model, explainer=S.explainer, model_name=nm,
                        )
                        cat_lbl, cls_lbl, color_lbl = risk_label(prob, S.threshold)
                        tier_lbl = risk_tier(prob, S.threshold)

                        S.new_patient_result = {
                            'pid':     new_pid or 'NEW',
                            'inputs':  dict(inputs),
                            'prob':    prob,
                            'cat':     cat_lbl,
                            ...
                            'tier':    tier_lbl,
                            'shap':    sv_new,
                            'add_to_roster': bool(add_to_roster),
                        }

                        if add_to_roster:
                            # Build a key-factor label consistent with the roster column.
                            if sv_new is not None:
                                top_idx = int(np.argmax(np.abs(sv_new)))
                                key_factor = '▲ ' + humanize_feature(feats[top_idx]) \
                                             if sv_new[top_idx] > 0 else \
                                             '▼ ' + humanize_feature(feats[top_idx])
                            else:
                                key_factor = '—'
                            S.new_patient_extras.append({
                                'Patient':    new_pid or 'NEW',
                                'Age':        str(int(inputs.get('age', 0))),
                        ...
                        st.error(f"We couldn't score this patient: {e}")

                # ── Render result if we have one ─────────────────────────────
                if S.new_patient_result is not None:
                    res = S.new_patient_result
                    thr = S.threshold

                    st.markdown("---")
                    st.markdown(
                        f"<p class='section-head'>Result for patient "
                        f"<code>{res['pid']}</code></p>",
                        unsafe_allow_html=True,
                    )
                    ...
                    fig_new = go.Figure(go.Indicator(
                        ...
                        title={'text': (f'Readmission Risk Score<br>'
                                        f'<span style="font-size:.75em;color:#64748b">'
                                        f'Ad-hoc patient | Threshold: {thr:.2f}</span>')},
                        number={'suffix': '%', 'font': {'size': 42, 'color': res['color']}},
                        gauge={
                            'axis': {'range': [0, 100], 'tickwidth': 1},
                    ...
                    fig_new.update_layout(height=320, paper_bgcolor='#ffffff',
                                          margin=dict(t=70, b=20, l=30, r=30))
                    st.plotly_chart(fig_new, use_container_width=True)
                    st.markdown(f"""
                    <div style='text-align:center;margin:12px 0 20px;'>
                      <span class='{res['cls']}'>{res['cat']}</span>&nbsp;&nbsp;
                      <span style='color:#64748b;font-size:14px'>
                        Probability: <strong>{res['prob']:.1%}</strong>
                      </span>
                    </div>
                    """, unsafe_allow_html=True)

                    # Local SHAP for the new patient
                    if res['shap'] is not None:
                        sv  = res['shap']
                        fts = S.feature_names
                        top_i = np.argsort(np.abs(sv))[::-1][:10]
                        t_f = [humanize_feature(fts[i]) for i in top_i]
                        t_v = [float(sv[i]) for i in top_i]
                        colors = ['#dc2626' if v > 0 else '#16a34a' for v in t_v]
                        fig_sh = go.Figure(go.Bar(
                            x=t_v[::-1], y=t_f[::-1], orientation='h',
                            marker_color=colors[::-1],
                            text=[f'{v:+.3f}' for v in t_v[::-1]], textposition='outside'))
                        fig_sh.update_layout(
                            title='Top factors for this patient (red = increases risk, green = decreases)',
                            xaxis_title='SHAP value', height=380,
                            plot_bgcolor='#f8fafc', paper_bgcolor='#ffffff',
                            margin=dict(l=210, r=90, t=50, b=40))
                        st.plotly_chart(fig_sh, use_container_width=True)

                        # Preventive checklist tailored to this patient
                        raw_top = [(S.feature_names[i], float(sv[i])) for i in top_i]
                        cl = generate_checklist(raw_top, res['cat'])
                        st.markdown("<p class='section-head'>Preventive action checklist</p>",
                                    unsafe_allow_html=True)
                        items_html = ''.join(f"<div class='chk-item'>&#9744;&nbsp;&nbsp;{item}</div>"
                                             for item in cl)
                        st.markdown(items_html, unsafe_allow_html=True)
                    else:
                        st.markdown(
                            "<div class='info-box'>SHAP explanation isn't available for this "
                            "patient — the score above is still valid. Try the Explainability "
                            "tab in the technical view to recompute SHAP if needed.</div>",
                            unsafe_allow_html=True,
                        )

                    if res.get('add_to_roster'):
                        st.markdown(
                            "<div class='success-box'>This patient has been added to the "
                            "roster below.</div>", unsafe_allow_html=True,
                        )

                    cclr1, cclr2 = st.columns([1, 5])
                    if cclr1.button("Clear result", key="clear_new_patient"):
                        S.new_patient_result = None
                        st.rerun()

            st.markdown("</div>", unsafe_allow_html=True)  # end new patient panel
            ...
            # Append any ad-hoc new patients the user opted to add to the roster.
            # These use negative _idx values so they don't collide with df row indices,
            # and the roster drill-in will recognise them as ephemeral entries.
            if S.new_patient_extras:
                extras_rows = []
                for k, ex in enumerate(S.new_patient_extras):
                    extras_rows.append({
                        'Patient':    ex['Patient'],
                        'Age':        ex['Age'],
                        'Key factor': ex['Key factor'],
                        'Risk %':     ex['Risk %'],
                        'Risk tier':  ex['Risk tier'],
                        '_idx':       -(k + 1),  # negative marker for ad-hoc patients
                    })
                roster_df = pd.concat(
                    [roster_df, pd.DataFrame(extras_rows)],
                    ignore_index=True,
                )
            ...
            if len(roster_df) > display_cap:
                ...
                st.markdown(
                    ...
                    f"<strong>{len(roster_df):,}</strong> matches, sorted by risk. "
                    f"Use search or filters to narrow down.</div>",
                    unsafe_allow_html=True,
                )
            else:
            ...
            st.markdown("</div>", unsafe_allow_html=True)  # close roster panel
