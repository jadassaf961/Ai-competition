...
.top-nav .nav-title {
  font-size: 17px; font-weight: 700; color: #0c4a6e; letter-spacing: -0.3px;
}
...
with nav_col1:
    ...
    <div style='display:flex;align-items:center;gap:12px;padding:6px 0;'>
      <div style='width:38px;height:38px;border-radius:10px;
                  background:linear-gradient(135deg,#0284c7 0%,#0ea5e9 100%);
                  display:flex;align-items:center;justify-content:center;
                  color:white;font-size:20px;box-shadow:0 3px 10px rgba(14,165,233,.25);
                  flex-shrink:0;'>🏥</div>
      <div>
        <div style='font-size:17px;font-weight:700;color:#0c4a6e;letter-spacing:-0.3px;'>
          CareInsight · Readmission Risk Platform
        </div>
        <div style='font-size:11px;color:#94a3b8;font-weight:500;'>
          Clinical decision support · 30-day readmission risk
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("<hr style='margin:0 0 18px;border:none;border-top:1px solid #e2e8f0;'>",
            unsafe_allow_html=True)

# ─────────────────────────────────────────────────────────────────────────────
# Technical view inline controls (replaces sidebar for technical mode)
# ─────────────────────────────────────────────────────────────────────────────
if S.view_mode == 'technical':
    with st.expander("📂  Data Upload & Configuration", expanded=(S.df is None)):
        up_col, cfg_col = st.columns([1, 1])
        ...
        with cfg_col:
            if S.df is not None:
                ...
                thr_col, split_col = st.columns(2)
                with split_col:
                ...
                if S.trained:
                    st.markdown(
                        f"<div class='info-box' style='margin-top:8px;font-size:12px;'>"
                        f"✅ Models trained &nbsp;|&nbsp; Best: <strong>{S.best_name}</strong> "
                        f"&nbsp;|&nbsp; Active: <strong>{S.chosen_name}</strong> "
                        f"&nbsp;|&nbsp; Threshold: <strong>{S.threshold:.2f}</strong> "
                        f"&nbsp;|&nbsp; SHAP: {'✅' if S.shap_computed else '❌'}"
                        f"</div>",
                        unsafe_allow_html=True,
                    )
...
if S.view_mode == 'technical':
    # Header
    st.markdown("""
    <div style='background:linear-gradient(135deg,#f0f9ff 0%,#e0f2fe 50%,#f0fdfa 100%);
                border:1px solid #bae6fd;border-radius:12px;
                padding:16px 24px;margin-bottom:16px;
                box-shadow:0 2px 12px rgba(14,165,233,.08);'>
      <div class='badge'>Healthcare AI &middot; Binary Classification</div>
      <div style='color:#0c4a6e;font-size:16px;font-weight:700;margin:6px 0 4px;'>
        Technical / Admin View
      </div>
      <div style='color:#475569;font-size:13px;'>
        Upload patient data, train ML models, surface SHAP explanations,
        audit fairness across subgroups, and export clinical memos.
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab_data:
        ...
        if S.df is None:
            st.markdown("""
            <div class='info-box'>
              ⬆️ Upload a CSV file using the <strong>Data Upload &amp; Configuration</strong> panel above to get started.<br>
    ...
    with tab_roc:
        ...
        if not S.trained:
            st.markdown("<div class='warn-box'>Train models first using the Data Upload &amp; Configuration panel above.</div>",
                        unsafe_allow_html=True)
        else:
            y_te = S.y_test
            ...
            if not S.trained:
                ...
                if st.button("⚖️ Run Fairness Audit"):
                    ...
                    if not S.sensitive_cols or S.X_test_raw is None:
                        st.markdown("<div class='warn-box'>Select fairness columns in the Data Upload &amp; Configuration panel above first.</div>",
                                    unsafe_allow_html=True)
                    else:
                        raw   = S.X_test_raw
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    # ── Clinical view sub-header ─────────────────────────────────────────────
    st.markdown("""
    <div style='background:linear-gradient(120deg,#f0f9ff 0%,#e0f2fe 100%);
                border:1px solid #bae6fd;border-radius:12px;
                padding:16px 24px;margin-bottom:20px;
                display:flex;justify-content:space-between;align-items:center;'>
      <div>
        <div class='badge'>Clinical Staff View</div>
        <div style='font-size:14px;color:#0c4a6e;font-weight:600;margin-top:4px;'>
          30-day Readmission Risk Analysis
        </div>
        <div style='font-size:12px;color:#64748b;margin-top:2px;'>
          Upload a patient dataset below to begin risk stratification.
        </div>
      </div>
      <div style='text-align:right;'>
        <div style='font-size:11px;color:#94a3b8;font-weight:500;letter-spacing:.6px;text-transform:uppercase;'>v1.0 · Clinical Release</div>
        <div style='font-size:12px;color:#475569;font-weight:600;margin-top:4px;'>For authorised clinical staff</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
